...
# nam <- paste("player2019")
# x <- merge(data.frame(Player=df_adv[[7]][,2],bpm_total=df_adv[[7]][,28]), df_game[[7-1]], by="Player")
# x <- merge(x, df_game[[7-2]], by = "Player", all.x=TRUE)
# x <- merge(x, df_game[[7-3]], by = "Player", all.x=TRUE)
# x <- merge(x, df_adv[[7-1]][, - c(1,3:7,length(df_adv[[7-1]]))], by = "Player", all.x=TRUE)
# x <- merge(x, df_adv[[7-2]][, - c(1,3:7,length(df_adv[[7-2]]))], by = "Player", all.x=TRUE)
# x <- merge(x, df_adv[[7-3]][, - c(1,3:7,length(df_adv[[7-3]]))], by = "Player", all.x=TRUE)
# assign(nam, data.frame(x))
...
# player2019 <- dplyr::rename(player2019, Player = Player, vorp_total = vorp_total, Year_t1 = Year.x, Pos_t1 = Pos.x,
#                     Age_t1 = Age.x, Tm_t1 = Tm.x, G_t1 = G.x, GS_t1 = GS.x, MP_t1 = MP.x, FG_t1 = FG.x,
#                     FGA_t1 = FGA.x, FGPer_t1 = FG..x, FG3_t1 = X3P.x, FGA3_t1 = X3PA.x, FG3Per_t1 = X3P..x,
#                     FG2_t1 = X2P.x, FGA2_t1 = X2PA.x, FG2Per_t1 = X2P..x, eFG_t1 = eFG..x, FT_t1 = FT.x,
#                     FTA_t1 = FTA.x, FTPer_t1 = FT..x, ORB_t1 = ORB.x, DRB_t1 = DRB.x, TRB_t1 = TRB.x, 
...
rm(list=setdiff(ls(), c("df","player2019"))) #remove all objects except for df
gc()
